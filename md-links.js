...
const statsOption = links => {
  return new Promise((resolve, reject) => { //  sobre uso de new Promise -- este es async
    let allLinks = links.map(link => link.href);  // async para cuando no quiero detener el flujo de ejecucion
    totalLinks += allLinks.length;  // la calculacion de los stats es sincrono 
...
const validateOption = links => {
  ...
  return new Promise((resolve, reject) => { //  esto está sobrecomplejizado, el promiseAll me crea y retorna 1 promesa
    let statusLinks = links.map(link => { //  arreglo de promesas para consolidar en 1 promesa
      // links.map(link => {
      return fetch(link.href).then(res => { //  retorna una promesa
        if (res.status === 200) {
